...
from config import ( ARGO_API_USERNAME, ARGO_API_KEY, 
    DEFAULT_TOP_K, STORE, QA_PATH, FERMILAB_SESSION_SECRET
)
import re
import time
import logging
...
class Utils():
    def extract_https_url(self, text):
...
class Metrics():
    def __init__(self, eval_model, api_client):
        
        self.client=FermilabAPIClient() if not api_client else api_client
        self.model = 'gpt-oss:20b' if eval_model else eval_model
        self.util_client = Utils()
        
        
    def relevant_refs(self, question, expected, contexts, references):
        '''
        ...
        '''
        expect = self.util_client.normalize(expected)
        ...
        if expect in references:
            ...
            resp=self.client.chat_completion(question=prompt, context=' '.join(contexts),  model=self.model)
    ...
    def correctness(self, main_question, expectation, output) -> float:
        '''
        ...
        try:
            ...
            resp = self.client.chat_completion(question=question, context=context, model =self.model) 
            match = re.search(r'(\d[\d\s]*\.?[\d\s]*)', resp)
...
class Evalutation():
    def __init__(self, port, experiment_name, data_path,model, top_k, keyword):
        ...
        self.model=model
        self.fermi_client=FermilabAPIClient()
    ...
    def create_validation_dataset(self):
        '''
        Creates a 'correctness' dataset where each entry is:
    ...
    def create_refs_dataset(self):
        '''
        Creates a 'relevant refs' dataset where each entry is:

        {
            'inputs': 
                {'question': "some question here"}, 
            'expectations': 
                {'expected_response': 'www.link.com/pathtowebsite'}
        }
        '''
        ...
        for row in refs_dataset.iterrows():
            ...
            try:
                ...
                link=self.util_client.extract_https_url(link)
    ...
    def evaluate(self,method):
        ...
        # Evaluates how 'correct' DUNEGPT's response is
        if method == 'correctness':
            ...
            for dictionary in self.eval_dataset:
                ...
                score_temp = self.metrics.correctness(question, expected, answer)
        ...
        # Evaluates how correct the retrieved chunks are
            ...
            for dictionary in self.ref_dataset:
                ...
                score_temp = self.metrics.relevant_refs(question, reference, contexts, references)
        ...
        # Evaluates how long it takes end-to-end
        elif method=='latency':
            duration = 0.0
            qa = pd.read_csv(QA_PATH)
            df = []
            ...
            return df, [{'score': duration/len(df}]
...
# Log the overall score and the per sample feedback/score 
