...
class ArgoAPIClient:
    ...
    def chat_completion(
        ...
        """Send a chat completion request to Argo API (generator interface)"""
            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "documentation for the DUNE experiment."
                    ),
                },
                {
                    "role": "user",
        ...
        # Streaming contract starts here
        ...
        try:
            ...
            yield f"[ERROR] Request to Argo API timed out after {timeout} seconds."

        except requests.RequestException as e:
            logger.error(f"Argo API request failed: {e}")
            yield f"[ERROR] Failed to get response from Argo API: {e}"
    ...
    def health_check(self) -> bool:
        ...
        try:
            gen = self.chat_completion(
                question="Hello",
                context="This is a test",
                timeout=10,
            )
            first = next(gen)
            return not str(first).startswith("[ERROR]")
        except Exception as e:
