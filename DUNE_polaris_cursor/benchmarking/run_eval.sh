...
for dir in "${data_dirs[@]}"; do
	...
	for model in "${models[@]}"; do
		k=2
		save="metrics/new_$model/keyword_top_k_${k}_${name}/relevant_refs"
		if [ ! -d "$save" ]; then
                        mkdir -p "$save"
                fi
		...
		save="metrics/new_$model/keyword_top_k_${k}_${name}/correctness"
		if [ ! -d "$save" ]; then
                        mkdir -p "$save"
                fi
		...
		save="metrics/new_$model/keyword_top_k_${k}_${name}/latency"
                if [ ! -d "$save" ]; then
                        mkdir -p "$save"
                fi
